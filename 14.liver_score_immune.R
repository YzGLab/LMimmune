...
load("~/Projects/ImmunePro/livemeta_cohort.Rdata")
#save(immunecohorts,immunephe,file="~/Projects/ImmunePro/immunecohorts_allRNASeqandPhe20220224.Rdata")

names(livemeta_cohort) 
LMiSscoreimmunelist<-list()
for(set in names(livemeta_cohort)){
  myexp<-livemeta_cohort[[set]]
  rt=as.matrix(myexp)
...
LMiSscore<-LMiSscoreimmunelist[[4]]
colnames(LMiSscore)
table(LMiSscore$Livermeta)
LMiSscore<-LMiSscore[LMiSscore$Livermeta=="Yes",]
LMiSscore$Cat_Response<-as.factor(LMiSscore$Cat_Response)
c<- ggplot(LMiSscore,
           aes_string(x="Cat_Response", y="LMiScore", 
                      fill ="Cat_Response",
                      color ="Cat_Response")) +
  geom_point(aes(shape=Cat_Response), 
...
c+stat_compare_means(method = 'kruskal.test') #kruskal.test
...
c+stat_compare_means(method = 't.test',
                     ...
                     #label = "p.signif",
                     symnum.args=list(),
                     comparisons = list(c("CR", "PR"),c("C4","C2"),c("C4", "C1")))
###

chisq.test(table(LMiSscore$Cat_Response,LMiSscore$group))

colnames(LMiSscore)
c<- ggplot(LMiSscore,
           aes_string(x="Immune.phenotype", y="LMiScore", 
                      fill ="Immune.phenotype",
                      color ="Immune.phenotype")) +
  geom_point(aes(shape=Immune.phenotype), 
             position = position_jitter(width = .15,height=-0.7),
             size=1)+
  xlab("")+
  #scale_shape_manual(values=shapes) +
...
###
LMiSscore<-LMiSscoreimmunelist[[1]]
write.csv(LMiSscore,file="IMvigor210_LMCIFscore.csv")
...
immuneresponse.path <-"Results/immuneresponse.path"
if (!file.exists(immuneresponse.path )) { dir.create(immuneresponse.path) }


pdf(file.path(immuneresponse.path,"repsonse_immunotherapy_liver_tissue.pdf"),width = 6.49, height = 6.58,onefile = F )
...
colnames(LMiSscore)
LMiSscore<-LMiSscore[LMiSscore$Liver_Visc_Met=="1",]
sur.cut<-surv_cutpoint(LMiSscore,time = "OS", event = "OS.event", variables = "LMiScore")
LMiSscore$group=ifelse(LMiSscore[,"LMiScore"]>(sur.cut$cutpoint[,1]),1,0)
...
#RFS
LMiSscore<-LMiSscore[LMiSscore$Liver_Visc_Met=="1",]
sur.cut<-surv_cutpoint(LMiSscore,time = "PFS", event = "PFS.event", variables = "LMiScore")
LMiSscore$group=ifelse(LMiSscore[,"LMiScore"]>(sur.cut$cutpoint[,1]),1,0)

kmfit<- survfit(Surv(PFS,PFS.event)~group,data=LMiSscore)
p.val<-Pvalue
HR <- paste("Hazard Ratio= ", round(HR,2), sep = "")
CI <- paste("95% CI: ", paste(round(low,2), round(up,2), sep = " - "), sep = "")
print(paste0(set," p= ",round(p.val,2)," ",HR))
